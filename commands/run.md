...
**`Theme` = the color labels in Trello, and the FIRST value is the ticket's type — mandatory.**
Exactly one of `security` · `logic` · `bug` · `feature` · `cleanup` · `copy`, copied from the
ticket's `Type:` line (the sync gives these fixed colors: security red, bug orange, logic yellow,
feature green, cleanup sky, copy purple — same color on every board and every project). After it,
optionally, one or two area words in the operator's language (`project screen` · `publish` ·
`artwork`), comma-separated. Reuse the area words already in the BOARD before inventing a new one
(each distinct name creates one more label and the board stops being scannable at a glance).
Without the column the sync still works, but the cards end up with no label: the operator asked on
2026-08-02 for a security tag «so people know», and on 2026-08-19 made the type tag mandatory on
every card, next to the project tag.
...
- Bad: `**APPROVED · full gate** · api 312 (=), web 88 (=) · exact diff (+69/-0) · sha256 of the PDF fixture verified`
- Good: `Customers can now download their invoice as a PDF from the portal · Example: you open an invoice and hit «Download PDF» — before, the button didn't exist · APPROVED, full gate (api 312, web 88) · bounded diff: +69 lines and one new endpoint`
...
0b. **Routing — decide which agents run, BEFORE launching any.** Extract the signals from the
   ...
   ```
   Signals: new_project:no · risk:no · ambiguous:no · architecture:yes · new_ui:no · trivial:no
