...
**Graphify (optional, only if `graphify-out/graph.json` exists at the repo root).** A second graph
that also covers **docs and configs** — files the code graph never sees. You have no Bash: read
`graphify-out/GRAPH_REPORT.md` (god nodes, communities, suggested questions) to orient in a domain
you don't know — one Read instead of several exploratory ones. It is a snapshot of the main repo:
verify anything you plan to cite against the real code. No `graphify-out/` → skip this, nothing changes.
...
(payment, submission, deletion) · **`Type:` exactly ONE of `security|logic|bug|feature|cleanup|copy`**
(the lead copies it to the BOARD and it becomes the card's colored tag in Trello) · assumptions if
you assumed something ambiguous · risks or dependencies between steps. If it's a bug, add `Reproduce:` (1-3 steps) and `Actual:` / `Expected:`.
