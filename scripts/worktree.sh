...
# node_modules and .env* are not tracked: a fresh worktree does not have them and the gate blows up
# on the first `npx tsc`. They are symlinked to the main repo — 0 bytes, same directory, nothing
# duplicated. A file the checkout already brought (e.g. a tracked .env.example) is never
...
bootstrap() {
  ...
  for src in "$repo"/node_modules "$repo"/*/node_modules; do
    [ -d "$src" ] || continue
    rel=${src#"$repo"/}
    [ -e "$wt/$rel" ] && continue
...
cmd_review() {
  ...
  if [ -d "$path" ]; then
    ...
    git -C "$path" clean -qfd -e node_modules -e '.env*' >&2 # leftovers from the previous verdict
  else
    mkdir -p "$(dirname "$path")"
