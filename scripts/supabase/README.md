...
You should see it download a browser and finish without errors.

Then tell every test where that browser is. Paste this line too:

```bash
echo 'export PLAYWRIGHT_MODULE=~/projects/claude-code-dev-loop/scripts/qa/pw-shim.mjs' >> ~/.bashrc
```

Without that line tests fail with `ERR_MODULE_NOT_FOUND`, even though the browser is installed.
(Why, if you care: reference section at the bottom.)
...
**3. `PLAYWRIGHT_MODULE` is not optional, and Chrome does not exist here.** A global Playwright
install is invisible to `import('playwright')`: ESM ignores `NODE_PATH`, so a test launched from a
project or a worktree dies with `ERR_MODULE_NOT_FOUND` while `npm ls -g` swears the package is
there. `scripts/qa/pw-shim.mjs` finds the module wherever it lives — point tests at it with
`PLAYWRIGHT_MODULE`. It also drops `channel: 'chrome'`, which fails on these hosts (no Google
Chrome, only the bundled chromium) with an error that reads like a broken test. `sbauth <project>
doctor` checks all this by actually opening a browser; asking whether the package resolves reports
a false green.
